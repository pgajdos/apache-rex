...
curl -s http://127.0.0.1:$AREX_PORT/administration  | grep 'welcome, administrator!' || exit_code=1

echo "[2] requests to other locations are declined"
curl -s http://127.0.0.1:$AREX_PORT/ | grep 'down to maintenance' || exit_code=2

echo "[3] requests from other address then 127.0.0.1 are declined"
curl -s http://[::1]:$AREX_PORT/administration | grep 'down to maintenance' || exit_code=3
