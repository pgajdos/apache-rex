...
for i in 1 2 3; do
  curl -s --location http://localhost:$AREX_RUN_PORT/users/user$i | grep "user$i's website" || exit_code=1
done
curl -s --location http://localhost:$AREX_RUN_PORT/users/user4 | grep 'new user' || exit_code=1
...
curl -s http://localhost:$AREX_RUN_PORT4/doc.html | grep '/doc.html created' || exit_code=2
sleep 2
curl -s http://localhost:$AREX_RUN_PORT4/doc.html | grep 'new page /doc.html'|| exit_code=2
...
curl -s http://localhost:$AREX_RUN_PORT5/~larry/welcome.html | grep "welcome to larry's website" || exit_code=3

echo "[4] redirecting anchors"
mkdir -p $AREX_RUN_DIR/htdocs-vh6
curl -s -i http://localhost:$AREX_RUN_PORT6/one.html \
  | grep 'Location: http://localhost:60086/two.html%23beginning' || exit_code=4
curl -s -i http://localhost:$AREX_RUN_PORT6/three.html \
...
curl -s http://localhost:$AREX_RUN_PORT6/five.html \
