...
curl -s http://localhost:$AREX_RUN_PORT/dir1/ | grep 'text index'    || exit_code=1
curl -s http://localhost:$AREX_RUN_PORT/dir2/ | grep 'php index'     || exit_code=1
curl -s http://localhost:$AREX_RUN_PORT/dir3/ | grep '404 Not Found' || exit_code=1
