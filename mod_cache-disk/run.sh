...
# was changed -> miss
curl -s -v -H 'Cache-Control: max-age=1' http://localhost:$AREX_RUN_PORT/index.html 2>&1 | grep 'X-Cache:.*MISS' || exit_code=3
...
curl -s -v -H "Cache-Control: no-cache" http://localhost:$AREX_RUN_PORT/ 2>&1 | grep 'X-Cache:.*MISS' || exit_code=5
curl -s -v -H "Cache-Control: no-cache" http://localhost:$AREX_RUN_PORT/ 2>&1 | grep 'X-Cache:.*MISS' || exit_code=5
curl -s -v -H "Cache-Control: no-cache" http://localhost:$AREX_RUN_PORT/ 2>&1 | grep 'X-Cache:.*MISS' || exit_code=5
