...
curl -s http://localhost:$AREX_PORT/~$USER/ | grep "hello from $USER user" || exit_code=1
...
curl -s http://localhost:$AREX_PORT1/~user1/ | grep 'hello from user1' || exit_code=2
echo "[3] public_html on another prefix,  UserDir disabled for user"
curl -s http://localhost:$AREX_PORT1/~root/ | grep '404 Not Found' || exit_code=3
...
curl -s http://localhost:$AREX_PORT1/~user2/ | grep 'hello from user2' && exit_code=4
