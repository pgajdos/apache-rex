...
<!--#endif -->
EOF
curl -s http://localhost:$AREX_PORT/ssi-by-extension.shtml | grep -v '^$' | tee 1.txt
...
curl -s http://localhost:$AREX_PORT/ssi-by-xbit.html | grep -v '^$' | tee 2.txt
...
<!--#endif -->
EOF
curl -s http://localhost:$AREX_PORT/nossi.html | tee 3.txt
...
<!--#exec cmd="ls" -->
EOF
curl -s http://localhost:$AREX_PORT/ssi-cgi.shtml | tee 4.txt
