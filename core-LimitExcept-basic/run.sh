...
curl -s -X POST                              http://localhost:$AREX_PORT/ | grep '401.*uthoriz'  || exit_code=1
curl -s -X POST -u admin:StrongAdminPassword http://localhost:$AREX_PORT/ | grep 'main index'    || exit_code=1
