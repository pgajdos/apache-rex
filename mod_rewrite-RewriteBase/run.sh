...
curl -s http://localhost:$AREX_PORT/subdir1/foo | grep 'bar SUBDIR1' || exit_code=1
...
curl -s http://localhost:$AREX_PORT/subdir2/subsubdir/foo | grep 'bar SUBSUBDIR' || exit_code=2
curl -s http://localhost:$AREX_PORT/subdir2/subsubdir/index.html | grep 'welcome in SUBSUBDIR' || exit_code=2

echo "[3] RewriteBase is missing"
mkdir -p $AREX_RUN_DIR/mirror/subdir3/
echo 'bar SUBDIR3' > $AREX_RUN_DIR/mirror/subdir3/bar.html
curl -s http://localhost:$AREX_PORT/subdir3/foo | grep '404 Not Found' || exit_code=3
...
curl -s http://localhost:$AREX_PORT/wordpress/index.phpa | grep 'wordpress index' || exit_code=4
curl -s http://localhost:$AREX_PORT/wordpress/notexisting.php | grep 'wordpress index' || exit_code=4
