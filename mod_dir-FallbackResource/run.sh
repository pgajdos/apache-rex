...
curl -s http://localhost:$AREX_PORT/dir/ | grep 'welcome to the gallery' || exit_code=1
# would got 404, fallback is chosen
curl -s http://localhost:$AREX_PORT/dir/not-existing.html | grep 'gallery thumnail' || exit_code=1
