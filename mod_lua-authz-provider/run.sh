...
curl -s http://127.0.0.1:$AREX_PORT/ | grep '401 Unauthorized' || exit_code=1

echo "[2] access allowed for ::1 user agent address"
curl -s http://[::1]:$AREX_PORT/ | grep 'private area index' || exit_code=2

echo "[3] access allowed for authenticated user"
curl -s -u john_doe:StrongPassword http://127.0.0.1:$AREX_PORT/ | grep 'private area index' || exit_code=3
