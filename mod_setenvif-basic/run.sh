...
curl -s http://localhost:$AREX_PORT/doc.html | grep 'common doc' || exit_code=1
curl -s --referer 'customer1.example.com' http://localhost:$AREX_PORT/doc.html | grep 'doc for customer1' || exit_code=1
