...
curl -s http://localhost:$AREX_RUN_PORT/doc/doc.html > $AREX_RUN_DIR/out-variables.txt
...
curl -s http://localhost:$AREX_RUN_PORT/doc/page.doc > $AREX_RUN_DIR/out-layout-page.txt
...
curl -s http://localhost:$AREX_RUN_PORT/system-info | grep 'GNU/Linux' || exit_code=3
...
curl -s http://localhost:$AREX_RUN_PORT/other-info | grep '404 Not Found' || exit_code=4
...
curl -X PUT -s http://localhost:$AREX_RUN_PORT/test-script/ | grep 'PUT method' || exit_code=5
curl -X GET -s http://localhost:$AREX_RUN_PORT/test-script/ | grep 'index content' || exit_code=5
curl -X GET -s http://localhost:$AREX_RUN_PORT/test-script/index.html?hi | grep 'GET method' || exit_code=5
