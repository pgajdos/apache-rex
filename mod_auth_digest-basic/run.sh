...
curl --digest -u john:WrongPassword -s http://localhost:$AREX_PORT/ | grep '401.*uthoriz' || exit_code=2
echo "[3] access denied for incorrect username"
curl --digest -u dave:StrongPassword -s http://localhost:$AREX_PORT/ | grep '401.*uthoriz' || exit_code=3
