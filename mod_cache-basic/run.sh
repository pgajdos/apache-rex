...
for i in $(seq 1 30); do
  curl -s -v http://localhost:$AREX_RUN_PORT/index.html 2>&1 | grep 'X-Cache\|Age'
  sleep 1
done
...
# was changed -> miss
curl -s -v -H 'Cache-Control: max-age=1' http://localhost:$AREX_RUN_PORT/index.html 2>&1 | grep 'X-Cache:.*MISS' || exit_code=2

echo "[3] miss with Cache-Control: no-cache"
curl -s -v -H 'Cache-Control: no-cache' http://localhost:$AREX_RUN_PORT/index.html 2>&1 | grep 'X-Cache:.*MISS' || exit_code=3
curl -s -v -H 'Cache-Control: no-cache' http://localhost:$AREX_RUN_PORT/index.html 2>&1 | grep 'X-Cache:.*MISS' || exit_code=3
curl -s -v -H 'Cache-Control: no-cache' http://localhost:$AREX_RUN_PORT/index.html 2>&1 | grep 'X-Cache:.*MISS' || exit_code=3
