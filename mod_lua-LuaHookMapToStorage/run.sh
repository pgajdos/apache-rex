...
curl -s http://localhost:$AREX_RUN_PORT/ | grep 'main index' || exit_code=1
# MISS
curl -s http://localhost:$AREX_RUN_PORT/images/apache_pb.png > $AREX_RUN_DIR/downloaded.png
file $AREX_RUN_DIR/downloaded.png | grep  'PNG image data' || exit_code=1
# MISS
curl -s http://localhost:$AREX_RUN_PORT/images/apache_pb.gif > $AREX_RUN_DIR/downloaded.gif
# HIT
curl -s http://localhost:$AREX_RUN_PORT/images/apache_pb.png > $AREX_RUN_DIR/downloaded.png
# HIT
curl -s http://localhost:$AREX_RUN_PORT/images/apache_pb.png > $AREX_RUN_DIR/downloaded.png
# MISS
curl -s http://localhost:$AREX_RUN_PORT/icons/image.png > $AREX_RUN_DIR/downloaded.png
# HIT
curl -s http://localhost:$AREX_RUN_PORT/icons/image.png > $AREX_RUN_DIR/downloaded.png
