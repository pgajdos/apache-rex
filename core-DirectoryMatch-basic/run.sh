...
curl -s http://localhost:$AREX_RUN_PORT/ | grep '403 Forbidden' || exit_code=1
curl -s http://localhost:$AREX_RUN_PORT/001/ | grep 'hello from 001' || exit_code=1
curl -s http://localhost:$AREX_RUN_PORT/subdir/002/ | grep 'hello from 002' || exit_code=1
curl -s http://localhost:$AREX_RUN_PORT/subdir/subsubdir/003/ | grep 'hello from 003' || exit_code=1
curl -s http://localhost:$AREX_RUN_PORT/0004/ | grep '403 Forbidden' || exit_code=1
