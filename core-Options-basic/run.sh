...
curl -s http://localhost:$AREX_RUN_PORT/b_link.txt | grep '403 Forbidden' || exit_code=1
echo "[2] FollowSymlinks set"
curl -s http://localhost:$AREX_RUN_PORT/foo/a_link.txt | grep 'A.TXT' || exit_code=2
echo "[3] Indexes not set"
curl -s http://localhost:$AREX_RUN_PORT/ | grep '403 Forbidden' || exit_code=3
echo "[4] Indexes set"
curl -s http://localhost:$AREX_RUN_PORT/foo/ | grep 'Index of /foo' || exit_code=4
echo "[5] Multiviews set"
curl -s http://localhost:$AREX_RUN_PORT/bar/c
