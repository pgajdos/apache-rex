...
curl -s http://localhost:$AREX_RUN_PORT2/foo.html | grep 'superfluous document' || exit_code=1
curl -s http://localhost:$AREX_RUN_PORT2/bar.html | grep 'old version document' || exit_code=1
