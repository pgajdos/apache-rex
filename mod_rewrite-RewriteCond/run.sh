...
curl -s http://127.0.0.1:$AREX_RUN_PORT/foo/bar/ | grep '127.0.0.1 site' || exit_code=1
curl -s http://127.0.0.2:$AREX_RUN_PORT/foo/bar/ | grep '127.0.0.2 site' || exit_code=1
...
curl -s http://127.0.0.1:$AREX_RUN_PORT/foo/bar/index.html?page=3 | grep 'PAGE 3' || exit_code=2
curl -s http://127.0.0.2:$AREX_RUN_PORT/foo/bar/index.html?page=2 | grep 'PAGE 2' || exit_code=2
