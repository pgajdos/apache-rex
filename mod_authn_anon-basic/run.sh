...
curl -u john:StrongPassword -s http://localhost:$AREX_PORT/foo/ | grep 'DATA' || exit_code=1
echo "[2] access granted to anonymous user"
curl -u anonymous:post@email.com -s http://localhost:$AREX_PORT/foo/ | grep 'DATA' || exit_code=2
...
curl -s http://localhost:$AREX_PORT/foo/ | grep '401.*uthoriz' || exit_code=4
echo "[5] access denied when no password given"
curl -u anonymous: -s http://localhost:$AREX_PORT/foo/ | grep '401.*uthoriz' || exit_code=5
echo "[6] access denied when password is not in email format"
curl -u anonymous:password -s http://localhost:$AREX_PORT/foo/ | grep '401.*uthoriz' || exit_code=6
