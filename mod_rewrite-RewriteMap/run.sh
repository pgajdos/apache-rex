...
curl -s --location http://localhost:$AREX_RUN_PORT/doc/jp/  | grep 'japanese doc index' || exit_code=1
curl -s --location http://localhost:$AREX_RUN_PORT/doc/fr/  | grep 'french doc index'   || exit_code=1
curl -s --location http://localhost:$AREX_RUN_PORT/doc/can/ | grep 'canadian doc index' || exit_code=1
