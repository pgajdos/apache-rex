...
curl -s -X GET  "http://localhost:$AREX_PORT/example_handler.lua?user=kremilek&action=povidla" \
  | grep 'user: kremilek' || exit_code=1
curl -s -X POST --data "user=vochomurka&action=povidla" http://localhost:$AREX_PORT/example_handler.lua \
  | grep 'action: povidla' || exit_code=1
curl -s -X PUT  http://localhost:$AREX_PORT/example_handler.lua \
